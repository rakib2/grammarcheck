 ...
 *
 * Lives at `/landing` while the existing `/` continues to serve the
 * signed-in quick-practice surface. Promote to root once we're ready to
 * route signed-out traffic here by default.
 *
    ...
    <header className="sticky top-0 z-30 border-b border-line/60 bg-bg/85 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3.5">
        ...
        <nav className="hidden items-center gap-6 text-[13px] text-ink-2 md:flex">
          ...
          <a href="#languages" className="transition-colors hover:text-ink">
            Languages
          </a>
        </nav>
...
// ── Demo strip ──────────────────────────────────────────────────
//
// Visual mockup using the actual component shapes from the app: a chat
// correction bubble + a tiny mastery grid. Static, no interactivity — just
// enough to communicate "this is what you'd be looking at."

function DemoStrip() {
  return (
    <section className="bg-paper-warm">
      <div className="mx-auto grid max-w-6xl gap-8 px-6 py-16 md:grid-cols-[1.1fr_0.9fr] md:py-20">
        {/* Left: a single correction in chat shape */}
        ...
        {/* Right: tiny mastery preview using the real GrammarTable shape */}
        <div className="flex flex-col gap-3">
          <p className="font-mono text-[11px] uppercase tracking-[0.12em] text-mute">
